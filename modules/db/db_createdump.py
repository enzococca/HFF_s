...
class RestoreSchema(object):
    ...
    def update_geom_srid(self, schema, crs):
        ...
        try:
            ...
            for r in res:
                tables.append(r[0])
                types.append(r[1])
            tables_and_types = dict(list(zip(tables, types)))
            for t, ty in list(tables_and_types.items()):
    ...
    def update_geom_srid_sl(self, crs):
        sql_query_string = ("SELECT f_table_name, f_geometry_column, geometry_type FROM {}".format('geometry_columns'))
        ...
        try:
            ...
            tables_and_types = dict(list(zip(tables, types_and_geom)))
            for t, ty in list(tables_and_types.items()):
